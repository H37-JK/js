...
await delay(3000);
await page.fill('#mb_id', 'these9904');
await page.fill('#page-login > form > fieldset > div > p.ctrl.ctrl-pw > input', 'star8903');
...
await delay(2000);
await page.check('#mypage_myinfo > table > tbody > tr:nth-child(3) > td:nth-child(1) > input[type=checkbox]');
...
for(let i = 2; i <= 100; i ++) {
    ...
    await page.goto(`https://ondisk.co.kr/index.php?mode=my_page&sm=regist&orderis=purchase_cnt&sort=DESC&code_cate=all&code=all&sch_state=&list_max=20&p=${i}`);
